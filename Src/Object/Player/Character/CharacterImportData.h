...
//Json呼び出し
inline void FromJson(const nlohmann::json& _j, CharacterImportData& _data)
{
	//名前
	if (_j.contains("name"))
	{
	...
	{
		_data.localPos.x = _j["localPos"].value("x", 0.0f);
		_data.localPos.y = _j["localPos"].value("y", 0.0f);
		_data.localPos.z = _j["localPos"].value("z", 0.0f);
	}
...
inline void FromCsv(std::stringstream& _ss, CharacterImportData& _s)
{
}